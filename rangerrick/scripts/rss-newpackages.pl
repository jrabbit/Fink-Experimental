...
if ($DOSCP)
{
	...
	for my $file (@FILES)
	{
		$file =~ s/.*\///;
		$movecommands .=
		  "; mv news/rdf/${file}.new news/rdf/${file}; chgrp fink news/rdf/${file}";
	}
...
sub get_cvs_log
{
	...
	if (
		open( CVSLOG,
			"CVS_RSH=/home/ranger/bin/ssh.sh cvs -d :ext:rangerrick\@cvs.sourceforge.net:/cvsroot/fink log @keys 2>$PREFIX/cvslog.log |"
		)
	  )
	{
		$count = 0;
...
sub make_rss
{
	  ...
	  )
	{
		...
		if ( $package->{'tree'} eq 'experimental' )
		{
			...
			$link =~ s/^${EXPDIR}\///s;
			$link =
			  'http://cvs.sourceforge.net/viewcvs.py/fink/experimental/' . $link
			  . '?rev=';
		...
		{
			$link =
			    'http://fink.sourceforge.net/pdb/package.php/'
			  . $package->{'package'} . '#'
			  . $package->{'version'} . '-'
			  . $package->{'revision'};
			$title =
	...
	for my $key ( sort keys %$split_rss )
	{
		...
		{
			my $savefile =
			  $DOSCP ? "fink-$key-$lctree.rdf.new" : "fink-$key-$lctree.rdf";
			$split_rss->{$key}->save( $TOPDIR . '/' . $savefile )
			  or die "can't save rss: $!\n";
...
#					"/usr/bin/code2html '$escaped' 2>/dev/null |"
	...
	for my $version (@versions)
	{
		...
		if (    $version->{'type'} eq 'info'
			and $version->get_info_filename() =~ /$DISTDIR\/([^\/]+)\// )
		{
			$version->{'tree'} = $1;
		}
		$version->{'date'} = $stat[9];
